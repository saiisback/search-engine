...
export default function SearchResults({ 
  ...
  useEffect(() => {
    ...
    const fetchResults = async () => {
      ...
      try {
        ...
        const res = await fetch(`http://54.254.162.138:10000/api/search/?query=${encodeURIComponent(query)}&search_engine=${selectedEngine}&num_results=${numResults}`)
  ...
  const fetchPageContent = async (url: string) => {
    ...
    try {
      const res = await fetch(`http://54.254.162.138:10000/api/content/?url=${encodeURIComponent(url)}`)
      ...
      <div className="grid gap-4">
        {results.map((result, index) => (
          <motion.div
            ...
            className={`group relative overflow-hidden rounded-lg border p-4 transition-all duration-300 hover:shadow-[0_0_15px_rgba(255,255,255,0.05)] ${
            ...
            {result.source && (
              <div className="mb-2 flex items-center">
                <span className={`rounded-full px-2 py-0.5 font-mono text-[10px] uppercase ${
                  result.source.includes('featured') 
